...
class Day(private val number: Int) {
    ...
    private fun runPart(partName: String, part: (() -> Any?)?) {
        if (part != null)
            terminal.println(
                "The result of $partName is ${
                    TextStyles.bold(
                        TextColors.brightCyan(
                            part.invoke().toString()
                        )
                    )
                }"
            )
    ...
    fun run() {
        ...
        val testFile = this::class.java.getResourceAsStream(inputFileName("_test"))
        if (testFile == null) {
            println(TextColors.brightRed("No test file provided. Skipping."))
        } else {
            input = testFile.bufferedReader().readText()
            testPart("part1", part1, expectPart1)
            testPart("part2", part2, expectPart2)
        }

        val actualFile = this::class.java.getResourceAsStream(inputFileName())
        if (actualFile == null) {
            println(TextColors.brightRed("No actual file a provided. Skipping."))
        } else {
            input = actualFile.bufferedReader().readText()
            println()
